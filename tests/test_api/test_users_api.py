...
async def user_token(async_client, verified_user):
    ...
    assert token is not None, "No access token returned for user login"
    return token
...
async def admin_token(async_client):
    ...
    form_data = {
        "username": "admin@example.com",  # Correct admin username
        "password": "adminpassword"      # Correct admin password
    }
    response = await async_client.post("/login/", data=urlencode(form_data), headers={"Content-Type": "application/x-www-form-urlencoded"})
    assert response.status_code == 200, f"Admin login failed: {response.text}"
    token = response.json().get("access_token")
    assert token is not None, "No access token returned for admin login"
    return token
...
async def test_retrieve_user_access_allowed(async_client, admin_user, admin_token):
    ...
    assert response.status_code == 200, f"Admin retrieve user failed: {response.text}"
...
async def test_update_user_email_access_allowed(async_client, admin_user, admin_token):
    ...
    assert response.status_code == 200, f"Admin update user email failed: {response.text}"
...
async def test_delete_user_does_not_exist(async_client, admin_token):
    ...
    assert delete_response.status_code == 404, f"Unexpected response: {delete_response.text}"
